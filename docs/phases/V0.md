...
### Failure modes
- **Given** a scrape against a host that escalates to WAF block on the HTTP lane, **when** the scrape runs, **then** the `WafDetector` classifies the response, policy caps HTTP retries, and the CLI persists `needsBrowser=true` on `DomainProfile` when the HTTP lane is exhausted (full `CrawlEngine` + browser lane TBD). Coverage: integration. Test: `packages/scraper/tests/engine-escalation.test.ts` (includes `http_block_routes_to_browser_or_persists`).
