...
- CLI / script entry: take a search URL or already-discovered listing URLs and produce normalized records.
...
- **Programmatic deduplication only** in V0 — deterministic keys + lexical similarity heuristics. No embeddings, no LLM. See [../architecture/dedup.md](../architecture/dedup.md).
...
- No UI, no auth, no workspaces, no AI. All of those start in V1.
...
flowchart LR
    ...
    Adapter -->|"Fetcher (got + AIA, fallback Playwright)"| Raw[Raw HTML/JSON]
...
- **Given** a `data/` directory populated by a prior scrape, **when** the same `pnpm clearbolt scrape <same-url>` is run again, **then** the count of `CanonicalDeal` files is unchanged AND the `MergeCandidate` queue depth does not grow. Coverage: smoke. Test: `apps/cli/tests/scrape-end-to-end.smoke.test.ts::rerun_yields_zero_new_canonicals`.
...
### Failure modes
- **Given** a scrape against a host that escalates to WAF block on the HTTP lane, **when** the scrape runs, **then** the `WafDetector` classifies the response, policy caps HTTP retries, and the CLI persists `needsBrowser=true` on `DomainProfile` when the HTTP lane is exhausted on search or listing fetches (full `CrawlEngine` + browser lane TBD). Coverage: integration. Test: `packages/scraper/tests/engine-escalation.test.ts` (includes `http_block_routes_to_browser_or_persists`); listing/search policy loop: `packages/scraper/tests/fetch-waf-policy.test.ts`.
