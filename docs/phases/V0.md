...
### Failure modes
- **Given** a scrape against a host that escalates to WAF block on the HTTP lane, **when** the scrape runs, **then** the `WafDetector` classifies the response, the `CrawlEngine` either escalates to the browser fetcher or persists `needsBrowser=true` in the `DomainProfile`, and the run does not loop forever on retries. Coverage: integration. Test: `packages/scraper/tests/engine-escalation.test.ts::http_block_routes_to_browser_or_persists`.
